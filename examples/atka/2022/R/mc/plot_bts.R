...
.get_ind_df <- function(M) {
    ...
    for (i in 1:n)
    {
      ...
      {
        A <- M[[i]]
        df <- data.frame( A[grep(paste0('Obs_Survey_',k), names(A))]  )
        names(df) <- c("Year", "obs", "pre","sd" )
...
#' @export
#' 
plot_ind <- function(M, xlab = "Year", ylab = "Index ", xlim=c(1990,2019), ylim = NULL, alpha = 0.1)
{
    xlab <- paste0("\n", xlab)
    ylab <- paste0(ylab, "\n")
