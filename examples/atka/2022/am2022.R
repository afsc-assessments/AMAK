...
modlyr   <- readList("mod2021/For_R.rep")
mod22   <- readList("mod22/For_R.rep")
...
rdf <- rbind(rdf,cbind(data.table(mod22$R,"2022 Assessment")))
...
rdf  <- rdf[yr>1990&yr<2023,.(yr,R=R,lb=lb,ub=ub,case)]
mnR <- mean(mod22$R)
...
ggplot(rdf,aes(x=yr-1,y=R,fill=case)) + xlab("Year class") + ylab("Age 1 recruits (thousands)") + 
       ...
       scale_x_continuous(breaks=seq(1990,2023,2)) +scale_fill_viridis(discrete=TRUE) +
...
#=====================
 names(M) 
 df <-as.data.frame(M$Obs_Survey_1) 
...
glimpse(df)
 ...
 ggplot(aes(y=Model,x=Year,ymax=ub,ymin=lb)) + theme_few(base_size=18)+
        scale_x_continuous(breaks=seq(1990,2022,2), limits=c(1990,2023)) + ylim(c(0,1.6e6)) +
 ...
 names()
AgeFitsSrv(mod22,rec_age=1,case_label=captyr)
...
# Fit to fishery data
#=====================
AgeFits(mod22,rec_age=1,case_label=captyr)
#AgeFits(mod20,rec_age=1,case_label="Model 20.0")
#AgeFits(modlyr,rec_age=1,case_label=caplyr)

dt <- data.table(modlyr$Obs_Survey_1[,1:4],M$Obs_Survey_1[,3])
...
plt_srr <-function(M,xlab="Spawning biomass",ylab="Recruits",main="Model 16.0b") {
  ...
  ggplot() + geom_path(data=df,aes(x=ssb,y=rec) ) +geom_text(data=df,aes(x=ssb,y=rec, label=yrs),size=4) + 
  xlab(xlab) + ylab(ylab) + ggtitle("Atka mackerel")  + theme_few(base_size=16) + geom_line(data=df2,aes(x=stock, y=rec),col="salmon",size=2) + theme_few() + 
   ...
   geom_point(data=df %>% filter(yrs>2021),aes(x=ssb,y=rec),size=7,color="yellow",alpha=.8)
}
plt_srr(mod22,main="Model 16.0b") + theme_few(base_size=18)

#-------------------------------------------------------------------------------
# run REMA for apportionments---------------------------------------------------
library(rema)
library(patchwork)
adf <- read_csv("rema_atka.csv")
adf %>% mutate(ul=biomass*1*exp(cv),ll=biomass/(1*exp(cv))) %>%
ggplot(aes(x=year,y=biomass,ymin=ll,ymax=ul)) + geom_errorbar() + geom_point(size=2,color="red") + theme_few() +
             coord_cartesian(ylim=c(0,800000)) +
               facet_grid(strata~.) + geom_smooth()
glimpse(read.csv("rema_atka.csv"))

atka<-prepare_rema_input(
  model_name = "Atka_mackerel",
  biomass_dat = read_csv("rema_atka.csv"),
  end_year = 2023,
 )
m <- fit_rema(atka)

atka2 <-prepare_rema_input(
  model_name = "Variable PE",
  biomass_dat = read_csv("rema_atka.csv"),
  end_year = 2024,
)
atka2$data$pointer_PE_biomass
atka2$data$pointer_PE_biomass<-rep(0,2)
atka2$map$log_PE
atka2$map$log_PE <-  as.factor(atka2$par$log_PE <- c(1,1))
plot_rema
...
plots$biomass_by_strata + ggthemes::theme_few() + coord_cartesian(ylim=c(0,800000)) + facet_grid(strata~.) + scale_fill_manual(values="salmon")
plots$biomass_by_strata$data %>% filter(year==2023)
plots$biomass_by_strata$data %>% filter(year==2023) %>% select(pred,strata)
names(plots$biomass_by_strata$data) 

plots$total_predicted_biomass + ylim(c(0,1900) ) + ggthemes::theme_few()
 mutate(name = fct_relevel(strata, 
            "Western Aleutian Islands", "north-east", "east", 
...
for (i in 0:15) {
  ...
  rn=paste0("mod22/retro/r_",i,".rep")
...
df  <- data.frame(Model = "Current", mod22$SSB )
...
AgeFits(retro10,rec_age=1,case_label="retro 3 assessment")
...
pdt <- data.table(read_csv("mod22/proj/spm_detail.csv")) %>% mutate(Alternative=as.factor(Alt))
names(pdt)
...
bfs <- pdt %>% filter(Sim<=30)
  ...
  thisyr=2022
 # head(bfs)
  bfss <- bfs %>% filter(Alternative==2) %>% select(Alternative,Yr,Catch,SSB,Sim) 
  pf <- data.frame(read.table(paste0(.projdir,"percentdb.out"),header=F) )
  names(pf) <- c("stock","Alt","Yr","variable","value") 
  pf
  p1
  p1 <- pf %>% filter(substr(variable,1,1)=="C",variable!="CStdn",Alt==2) %>% select(Yr,variable,value) %>% 
...
p1 <- pdt %>% filter(Alternative==1) %>% mutate(Year=Yr) %>% ggplot(aes(x=Year,y=Catch),fill="salmon") + 
        expand_limits(y = 0) + 
       ...
       p1 <- p1 + geom_line(data=mtmp,aes(x=Yr,y=Catch),size=.2,col="grey40") ;p1
       tail(mtmp)

  p1 <- pdt %>% filter(Alternative==1) %>% mutate(Year=Yr) %>% ggplot(aes(x=Year,y=SSB)) + 
        expand_limits(y = 0) + 
        scale_y_continuous(labels = comma) +
        ggplot2::stat_summary(fun.ymin = function(x) quantile(x, 0.05), fun.ymax = function(x) quantile(x, 0.95), geom = "ribbon", alpha = 0.25, colour = NA) +
        ggplot2::stat_summary(fun.y = function(x) quantile(x, 0.5), geom = "line", lwd = 1) +
